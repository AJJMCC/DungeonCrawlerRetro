...
public class MapGenScript : MonoBehaviour {
    ...
    private string TagToGive;

    private float MapLevelY;
    ...
    public void RecieveCommand(Transform NewTrans, Texture2D NewMap, float Level)
    {
        trans = NewTrans;
        Map = NewMap;
        TagToGive = Level.ToString();
        MapLevelY = NewTrans.transform.position.y;
			...
			for (int j = 0; j < width; j++) {
				...
				if (pixelColor == Color.white) {
					if(prefabClusterWhite.Length!=0) {
						...
						inst.transform.position = new Vector3(j* multiplierFactor, MapLevelY, i* multiplierFactor);
                        inst.tag = TagToGive;
                    }
				}
